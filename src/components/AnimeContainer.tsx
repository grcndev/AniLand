  ...
  return (
    <>
      {rowsData?.map((row, index) => {
        return (
          <div key={index} className="group items-center justify-center flex flex-wrap 2xl:px-24 mt-16 grid-cols-1 gap-2 ml-90">
            <div className="flex flex-col xl:mx-48">
              <h2 className="text-txtcatg text-xl ml-3 sm:-mb-5 font-medium ">
                {row.title}
              </h2>
              <Link href={`/search/anime?`}>
              <span className="flex cursor-pointer align-bottom justify-end mr-6 -mb-2 xs:text-xs sm:text-sm relative text-viewall hover:text-txtcard">view all</span>
              </Link>
