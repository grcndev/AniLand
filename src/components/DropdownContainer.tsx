...
const DropdownContainer = ({ contents, category, onSelect }: DropdownContainerProps) => {
  ...
  const selectedValues = searchParams.getAll(category).map((value) => value.toLocaleLowerCase())

  const handleSelect = (value: string | number) => {
    const searchedContent = typeof value === "number" ? value.toString() : value.toLowerCase()
  ...
  const handleDelete = (value: string | number) => {
    ...
    const updatedValues = searchParams.getAll(category).filter((v) => v.toLowerCase() !== searchedContent);
    ...
    <div className="overflow-hidden overflow-y-auto max-h-96 bg-white rounded-md mt-2 absolute font-medium w-44 text-sm z-40 shadow-lg">
      {contents.map((content, index) => {
        const isSelected = selectedValues.includes(
          typeof content === "number" ? content.toString() : content.toLowerCase()
        );
        ...
        return (
          ...
          >
            ...
            {isSelected && (
              ...
              >
              <svg className="flex items-center justify-center ml-[5px] mt-1"  aria-hidden="true" focusable="false" data-icon="check" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 700">
